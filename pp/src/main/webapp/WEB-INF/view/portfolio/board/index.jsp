...
function moveWrite() {
	...
	location.href='/pp/portfolio/board/write.do';
	</c:if>
	<c:if test="${empty authUser}">
	alert('로그인 후 이용가능합니다.');	
	$(".login_info").toggle();
		...
		<div class="bbs">
			...
			<form><input type="hidden" name="user_no" value="${authUser.user_no }"></form>
				...
				<c:forEach var="vo" items="${list}">							
					<tr style='cursor:pointer;' onclick="location.href='detail.do?board_no=${vo.board_no }'">
						<td class="first"><input type="checkbox" name="nos" id="no" value="${vo.board_no }"/></td>
						<td>${vo.board_no }</td>
						<td class="txt_l">${vo.board_title} [${vo.commentCount }]</td>
						<td class="writer"> ${authUser.user_name }</td>
						<td class="date">${vo.board_regdate }</td>
